...
| 배지 글자 | 웨이브 파도체 — 숫자 82px · `월` 74px (세로로 길어 작게), 흰색. 글자층만 2px 내려 원 중심에 맞춤 |
| 제목 `사장님 달력` | 웨이브 파도체 85px / `letter-spacing:-0.01em` / **`#000000`** (태그라인만 `#3E4856`) |
...
| 날짜 숫자 | Pretendard 25px, `#3E4856` (일요일·공휴일 `#F96C5A`) |
...
**노랑 하이라이트 규칙** (`buildCalendarCard` 안의 `isLit` / `linked` / `span` / `dupText`)

1. **노랑이 켜지는 날** (`isLit`) — 셋 중 하나면 켜져요:
   - `text` — 이벤트명이 있는 날
   - `holiday` — 공휴일 체크 (날짜 빨강 + 노랑)
   - `mark` — 이벤트·공휴일 없이 날짜만 칠하는 수동 토글. 시안의 9·10·11·12가 여기 해당.

   입력 패널에서 이벤트나 공휴일이 있으면 **강조 (자동)** 으로 바뀌며 체크박스가 잠깁니다
   (`autoLit`).

2. **여러 날에 걸친 행사는 하나로 이어 그려요** (`linked`). 이어지는 조건 세 가지:
   - **공휴일이 연달아 있는 경우** — 추석·설 연휴. 이름은 보통 첫날에만 적으니
     텍스트가 없는 날도 이어져요.
   - `end`에 마지막 날 지정 — `'9/26'` · `'26'` · `'9월 26일'` 모두 파싱(`calParseEndDay`)
   - **같은 이벤트명을 연달아 입력** — 24·25·26 모두 `'추석'` → 한 밴드로 병합.
     이름은 첫날에만 나와요 (`dupText`가 둘째 날부터 같은 텍스트를 숨김).

   > ⚠️ **`mark`만 연달아 찍은 건 이어붙이지 않아요.** 시안의 9·10·11·12가 각각 떨어진
   > 동그라미라서 — `강조`는 "이 날짜 하나"를 찍는 용도예요. 연휴처럼 이어붙이려면
   > 공휴일을 체크하거나 `end`를 쓰세요.

   이름이 다르고 공휴일도 아니면 병합하지 않아요 (`A`/`B` → 동그라미 2개).
   공휴일이면서 이름이 서로 다르면 **이어지되 이름은 각각** 표시돼요 (추석 / 대체휴일).
...
- 월요일 시작이라 `calFirstWeekday`는 `(getDay()+6)%7` 로 0=월 … 6=일. 일요일 판정은 `col === 6`.
  **일요일은 빨강일 뿐 `holiday`가 아니라서 자동으로 노랑이 켜지지 않아요.**

### 폰트

| 용도 | 폰트 | 파일 |
| --- | --- | --- |
| 제목 (`9월 사장님 달력` — 배지 + 텍스트) | 웨이브 파도체 (`WavvePADO`) | `./달력/웨이브파도체/WavvePADO-Regular.woff2` |
| 요일 · 날짜 · 행사 이름 · 태그라인 | Pretendard (CDN) | — |

웨이브 파도체는 웨이브(Content Wavve)가 배포하는 무료 서체예요.
**상업적 이용 포함 조건 없이 자유**, 폰트 판매와 BI/CI 용도만 금지 (`fsType 0`).
공개 저장소·Pages에 woff2를 그대로 올려도 됩니다. 라이선스 원문은
`달력/웨이브파도체/LICENSE-웨이브파도체.pdf`.

> 배포본 원본 폴더(`달력/웨이브 파도체/`)는 폴더명에 **공백**이 있어 URL로 쓰기 번거로워요.
> 공식 웹폰트 woff2만 공백 없는 `달력/웨이브파도체/`로 복사해 CSS가 이쪽을 참조합니다.
> 원본 세트(otf/ttf/eot/woff)와 `웨이브 문구세트/` 스티커 이미지는 로컬 전용(.gitignore).

> 새 폰트로 바꾸면 **세로 메트릭이 달라 위치를 다시 잡아야 해요.** 윤고딕 → 웨이브 파도체 때는
> 제목을 `top:-2px` → `top:4px`, 배지 글자층을 `top:0` → `top:2px`,
> 제목 `margin-left`를 17 → 10으로 조정해 시안의 잉크 위치(제목 y92~167, 배지와 간격 17)를 맞췄습니다.
